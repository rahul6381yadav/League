  ...
  useEffect(() => {
    const fetchEvents = async () => {
      ...
      try {
        ...
        const [upcomingRes, ongoingRes, pastRes, statsRes] = await Promise.all([
          ...
          axios.get(`${backendUrl}/api/v1/club/stats`, {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2">
              <div className="p-6 rounded-lg shadow-md bg-mirage-200 dark:bg-mirage-800 mb-6">
                  ...
                  {Array.from({ length: daysInMonth }).map((_, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {hasEvents && (
                          <div className={`h-2 w-2 rounded-full mx-auto mt-1 ${getEventColor(date)}`}></div>
                        )}
                      </div>
                    );
                  })}
                ...
                {selectedDate && (
                  <div className="mt-6">
                    <h3 className="font-semibold text-mirage-950 dark:text-mirage-50 mb-3">
                    ...
                    {events.length > 0 ? (
                      <div className="space-y-3">
                        {events.map((event) => (
                          <div
                            key={event._id}
                            className="p-3 bg-white dark:bg-mirage-700 rounded-lg shadow-sm hover:shadow-md transition-shadow border-l-4 border-blue-500"
                            onClick={() => navigate(`/event/${event._id}`)}
                          >
                            <h4 className="font-medium text-mirage-950 dark:text-mirage-50">{event.name}</h4>
                            <div className="flex justify-between mt-2 text-sm">
                              <span className="text-mirage-600 dark:text-mirage-400 flex items-center">
                      ...
                      <div className="py-4 text-center text-mirage-600 dark:text-mirage-400">
            ...
            <div className="space-y-6">
                ...
                {upcomingEvents.length > 0 ? (
                  <div className="space-y-4">
                    {upcomingEvents.slice(0, 4).map((event) => (
                      <div 
                        key={event._id}
                        className="bg-white dark:bg-mirage-700 p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow cursor-pointer"
                        onClick={() => navigate(`/event/${event._id}`)}
                      >
                        <div className="flex justify-between items-start">
                          <div>
                            <h3 className="font-medium text-mirage-950 dark:text-mirage-50">{event.name}</h3>
                            <div className="flex items-center mt-2 text-sm text-mirage-600 dark:text-mirage-400">
                              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                              </svg>
                          ...
                          <div className={`px-2 py-1 text-xs font-medium rounded-full ${
                            new Date(event.date).getTime() - new Date().getTime() < 86400000 * 3
                        ...
                        <div className="flex justify-between mt-3 text-sm text-mirage-600 dark:text-mirage-400">
                ...
                {ongoingEvents.length > 0 ? (
                  <div className="space-y-4">
                    {ongoingEvents.map((event) => (
                      <div 
                        key={event._id}
                        className="bg-white dark:bg-mirage-700 p-4 rounded-lg shadow-sm hover:shadow-md transition-shadow border-l-4 border-yellow-500 cursor-pointer"
                        onClick={() => navigate(`/event/${event._id}`)}
                      >
                        <h3 className="font-medium text-mirage-950 dark:text-mirage-50">{event.name}</h3>
                        <div className="flex justify-between mt-2 text-sm text-mirage-600 dark:text-mirage-400">
                          <span className="flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                          ...
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              // Handle quick action like marking attendance
                              navigate(`/event/${event._id}/attendance`);
                            }}
                          >
                            Mark Attendance
                          </button>
                  ...
                  <div className="bg-white dark:bg-mirage-700 p-6 rounded-lg text-center">
                    <p className="text-mirage-600 dark:text-mirage-400">No ongoing events at the moment</p>
                  </div>
                )}
              </div>
              ...
              {/* Quick Links */}
              <div className="p-6 rounded-lg shadow-md bg-mirage-200 dark:bg-mirage-800">
                <h2 className="text-lg font-bold text-mirage-900 dark:text-mirage-50 mb-4 flex items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div className="space-y-2">
                  <a 
                    href="/members" 
                    className="flex items-center p-3 bg-white dark:bg-mirage-700 rounded-lg hover:bg-mirage-100 dark:hover:bg-mirage-600 transition-colors"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-3 text-blue-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
                    </svg>
                    <span className="text-mirage-950 dark:text-mirage-50">Manage Members</span>
                  </a>
                  <a 
                    href="/attendance" 
                    className="flex items-center p-3 bg-white dark:bg-mirage-700 rounded-lg hover:bg-mirage-100 dark:hover:bg-mirage-600 transition-colors"
                  >
                    ...
                    <span className="text-mirage-950 dark:text-mirage-50">Attendance Reports</span>
                  </a>
                  <a 
                    href="/messages" 
                    className="flex items-center p-3 bg-white dark:bg-mirage-700 rounded-lg hover:bg-mirage-100 dark:hover:bg-mirage-600 transition-colors"
                  >
                    ...
                    <span className="text-mirage-950 dark:text-mirage-50">Messages</span>
                  </a>
                  <a 
                    href="/resources" 
                    className="flex items-center p-3 bg-white dark:bg-mirage-700 rounded-lg hover:bg-mirage-100 dark:hover:bg-mirage-600 transition-colors"
                  >
                    ...
                    <span className="text-mirage-950 dark:text-mirage-50">Resources</span>
                  </a>
                </div>
              </div>
            </div>
